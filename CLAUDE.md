...
**ALWAYS（2026-07-23 使用者明確要求）：只要一個案件確認有已發包項目，該案件財務管理頁面就必須有一份「本案發包明細」視覺化表格——不是等有東西才臨時補，是每次確認發包當下就要有。若頁面本身還沒有任何「本案發包明細」視覺化呈現（既沒有 Notion 篩選檢視，也沒有原生 table 區塊），一律用原生 Notion `table` 區塊（非獨立子資料庫）畫一份，欄位：廠商名稱/工種/成本(含稅)/狀態/備註：**
- 每次在發包管理新增/更新該案件的記錄時，同步在這個 table 區塊 append 一行或更新對應列，不等使用者提醒
- **NEVER** 用 curl 直接傳中文 JSON payload（`-d '...'` 或 `-d @file`）給 Notion API——這個 Windows 環境會把中文字元送壞（親測 2026-07-23，「本案發包明細」變成亂碼），一律改用 Node.js `fetch`（`.mjs` 檔，`node 檔名.mjs` 執行）處理含中文的 Notion API 請求
- 若該案件已經有「本案發包明細」是發包管理資料庫的**篩選檢視**（Notion UI 建立的 linked database view，只需設定一次、之後自動即時更新，不需要每次手動維護，跟原生 table 區塊是兩種不同機制）——優先沿用篩選檢視，不要疊加原生 table 區塊造成重複

**發包管理資料庫（`3337aebd-089f-80d1-aa42-f85b80c119b9`）與案件財務管理頁面內的「本案發包明細」inline 表格必須同步：**
...
### 動手前先查踩坑記錄原則（已犯錯2次以上：發包管理relation同步、Notion中文寫入錯字，2026-07-23）

**問題**：踩坑記錄有寫進 memory，但只有「被使用者抓到才回頭查」，不是動手前主動查，導致同一類錯誤在不同案件反覆發生。

- **ALWAYS**：對已知有踩坑記錄的高風險流程（發包管理建立/更新、Notion 資料庫寫入含中文自由文字欄位、Excel 填值等）動手前，先查 Claude Code 自動記憶系統（session 開頭已載入的 `MEMORY.md` 索引，位於 `~/.claude/projects/<專案>/memory/`，非 `000_Agent/memory/`）裡相關的 `feedback_*` 記憶並照著做，不要等寫完才驗證
- **ALWAYS**：寫入 Notion 的 relation 若資料庫文件標示「雙向同步」以外的情況（如發包管理「專案」→ 案件財務管理「發包項目」是單向），建立記錄當下就兩邊都填，不要分兩次
- **ALWAYS**：寫入中文自由文字欄位（品項名稱、規格、備註等）後，逐字讀回比對，不要單憑印象輸入 unicode escape；若不確定某字的正確碼位，用 `python3 -c "print(hex(ord('字')))"`（配合 UTF-8 輸出）查證後再寫，不要用「猜錯→讀回→再猜→再讀回」的試錯迴圈
- 每日 `/morning` 執行時，加入「踩坑提醒」段落：重新掃過自動記憶系統裡所有 `feedback_*` 型記憶，列出近期（30天內）新增或被引用過的踩坑記錄，提醒使用者尚有哪些規則容易被忘記套用
- 詳見自動記憶系統中 `feedback_dispatch_cost_relation_sync`、`feedback_notion_chinese_typo_verify`
