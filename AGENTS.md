...
- 既有標單轉檔、分類或加公式不代表授權改版：保留原檔，於副本依指定範例處理；原欄寬、列高、字型與合併儲存格不得自行重設。工種分頁集中項目時，連同續列及各來源列高整段複製；只按授權加顏色，交付前核對尺寸及實際顯示。
- 工種詢價包須含可獨立填報的空白標單、對應圖說、規範及來源清單；不能只完成圖規分類就宣稱詢價包完成。依指定既有案例核對實檔；標單單價可填、複價公式不得依賴未附的主檔或其他工種，交付前實際填價測試並恢復空白。
