...
#### 2.5 確認案件財務管理頁面的「本案發包明細」表格已同步

> ⚠️ **2026-07-08 重大發現**：在至少一個案件（龍潭1F門面裝修工程）中，使用者看到的「本案發包明細」表格，實際上是**發包管理資料庫的過濾檢視**（用 `專案` relation 篩選該案件），欄位跟發包管理一模一樣（廠商名稱/工種/成本/狀態/是否已發包），**不是**獨立的子資料庫。這代表只要 STEP 8-2 建立發包管理記錄時 `專案` relation 有正確填，資料就會自動出現在這張表格，**不需要額外寫入任何地方**。

**判斷步驟（依序執行，不要跳步）：**

1. 完成 STEP 8-2 建立發包管理記錄（含正確的 `專案` relation）後，**先詢問使用者**：「案件財務管理頁面的『本案發包明細』表格是否已經看到這筆新資料？」
2. **若使用者確認已看到** → 完成，不用做任何事，直接跳到 STEP 8-3 回報
3. **若使用者說沒看到** → 才需要進一步排查：
   a. 用 `API-get-block-children` 讀取 `$PROJECT_ID`（案件財務管理頁面）的 block children，找 `type: "child_database"` 的區塊
   b. 若找到 → 嘗試 `API-retrieve-a-database` 讀取該區塊的 schema
      - 若成功且欄位是「工種/品項、廠商、合約金額、已付金額、狀態、備註」→ 這是獨立子資料庫，比照下方「獨立子資料庫寫入方式」新增一筆
      - 若失敗，錯誤訊息為 `does not contain any data sources accessible` → **這通常代表該資料庫本身在 Notion 後端資料來源損毀（孤兒物件），不是權限問題，重試授權/連接沒有用**。**絕對不要自行建立一個新的同名資料庫**（會造成頁面上出現重複表格，2026-07-08 已發生過一次並造成使用者困擾）。改為：
        - 告知使用者這個技術限制，並請使用者在 Notion 裡「複製」該表格（Duplicate）產生一個資料來源健康的新表格，複製完成後把新表格分享給整合，再回頭手動新增這一筆，**或**
        - 若使用者不想處理，先確認發包管理記錄本身已正確建立、`專案` relation 已填，告知使用者「本案發包明細這張表格暫時同步不到，但發包管理資料庫本身資料正確，之後表格修好可以再手動補」，不要為了硬要寫入而反覆嘗試同一招超過 3 次
   c. 若確認是「過濾檢視」型態（欄位與發包管理相同）→ 說明給使用者聽，不需另外寫入，跳到 STEP 8-3 回報

**獨立子資料庫寫入方式**（僅當 3-b 確認欄位是「工種/品項、廠商、合約金額、已付金額、狀態、備註」時才用）：
...
3. **不含稅金額為基準**（僅適用 STEP 4 工程報價管理系統的品項單價），有含稅版本寫入備註；**STEP 8 發包管理／本案發包明細的「成本」「合約金額」一律用含稅金額**，兩者標準不同不要混用
