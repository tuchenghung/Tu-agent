...
- 從現有選項中找最符合的
- 若無完全符合，新增選項並告知使用者
- ⚠️ **務必用完整案件名稱**（與案件管理系統「專案」標題一致，例如「龍潭1F門面裝修工程」），**不要用簡稱**（例如「龍潭1F」）。案件財務管理頁面的過濾檢視是用完整名稱篩選，簡稱會導致品項在該頁面看不到（2026-07-08 曾發生此問題）
...
**廠商名稱格式**：`{工程案件名稱}｜{廠商簡稱}{工種簡稱}`（**所有發包管理記錄一律套用此格式，不可只填廠商名稱**）
- 廠商簡稱：同 STEP 3 編號簡稱
- 工種簡稱：取工種主要關鍵字（如「乾盤管」「輕隔間」「地磚」）
- 範例：`南港GTP實驗室修改工程｜嘉祿乾盤管`、`羅東聖母S棟｜極鋼鐵工`
- **建立前先查詢**（依 `專案` relation）同案件是否已有相同廠商的其他發包記錄：
  - 若無 → 直接套用上述格式
  - 若有 → 在後面加區分後綴，如 `(主批)`、`(大門上面追加)`、`(加購)`，避免兩筆撞名混淆（2026-07-08 曾發生同案件同廠商兩筆都叫「美迪亞玻璃」的問題）
- 若日後發現同案件內有舊記錄仍是「純廠商名稱」（未套用此格式，多半是舊版流程留下的），**主動一併改成統一格式**，不要放著不管
...
#### 2.5 同步案件財務管理頁面的視覺化「本案發包明細」表格

> ⚠️ **2026-07-23 三度修正**：2026-07-08 版指示「找到獨立子資料庫就寫入」、2026-07-14 版改成「relation 自動反映即可，不建表格」，兩者都不是使用者要的。**2026-07-23 使用者明確要求：只要案件確認有已發包項目，該案件財務管理頁面就必須有一份看得到的「本案發包明細」表格，不能只靠 relation 屬性欄位。** relation/rollup 資料正確是必要條件，但不是使用者驗收的標準——他要在頁面正文看到表格。

**執行方式：**

1. 完成 STEP 8-2 建立發包管理記錄（含正確的 `專案` relation）後，**用 `API-get-block-children` 檢查該案件財務管理頁面**是否已有視覺化表格：
   - 有 Notion **篩選檢視**（linked database view，篩選發包管理的「專案」= 本案）→ 資料本來就會自動出現，不用動作
   - 有原生 `table` 區塊 → 在裡面 append 一行（或更新對應列，若同廠商同工種記錄已存在）
   - 兩者都沒有 → 用原生 Notion `table` 區塊建一份，欄位：廠商名稱/工種/成本(含稅)/狀態/備註，第一列填本次建立的記錄
   - 有 `type: "child_database"` 且 API 讀不到內容（`does not contain any data sources accessible`）→ 這通常是使用者手動在 Notion UI 建的表格、還沒對整合開放 Connections，或是 Notion 多資料來源模型的已知限制；**不要嘗試寫入**，改用原生 `table` 區塊補一份，並告知使用者那個表格權限問題待解決
2. **NEVER 用 curl 傳中文 payload**——這個環境會把中文字元送壞，一律用 Node.js `.mjs` 腳本 + `fetch`（複製 `add_dropbox_link_to_notion.mjs` 模式），寫入前用暫存 `_tmp_*.mjs` 檔、執行完立刻刪除
3. 寫入後用 `API-get-block-children`／查詢結果逐字核對中文內容有無錯字或亂碼，發現異常立即 `archived: true` 封存重建，不要嘗試用 PATCH 修正亂碼

#### 2.6 將報價單檔案從供應商報價資料夾移到「已發包」資料夾

> 2026-07-23 使用者要求：YUSHI 案件確認發包後，報價單 PDF 要從報價資料夾（STEP 5.5 歸檔的位置，資料夾名稱依專案而異，可能是 `F供應商報價與資料/` 或 `J供應商資料/` 等）**移動**（非複製）到專案裡名稱含「已發包」關鍵字的資料夾（例如 `N廠商已發包資料/`）。

1. 掃描專案資料夾，找出名稱包含「已發包」的資料夾（`fs.readdirSync` 比對），若找不到就詢問使用者正確資料夾名稱
2. 用 `fs.renameSync`（同一個 Dropbox 帳號內移動，不要 copy+delete）把 STEP 5.5 歸檔的檔案搬過去，檔名不變
3. Dropbox 分享連結是綁定檔案本身、不是路徑，搬移後**連結不會失效**，不需要重新取得
4. 但 STEP 6 寫進 Notion 文件的「歸檔路徑」文字段落需要同步更新成新路徑（用 Node.js `fetch` PATCH 該 block，見 [[feedback_curl_chinese_encoding]]，不要用 curl）
5. 若該報價單有多個工作文件中心頁面共用同一份檔案（少見），要逐一更新
...
3. **不含稅金額為基準**（僅適用 STEP 4 工程報價管理系統的品項單價），有含稅版本寫入備註；**STEP 8 發包管理的「成本」欄位一律用含稅金額**，兩者標準不同不要混用
4. **所有欄位不留空**，能查到的資料一定補上
5. **不要單憑報價單上的制式文字判斷含稅/未稅**（例如「上列單價未含加值稅5%」常是範本樣板文字，不一定反映實際成交條件）。若金額是否含稅有疑義，**直接問使用者**確認，不要自行套用文字推論後就寫入資料庫（2026-07-08 曾誤判導致金額寫錯，經使用者糾正）
6. **工種 select 選項**從資料庫現有選項中選，不自創新選項（除非使用者確認）
7. **單位 select 選項**同上，從現有選項選
8. **一價多項目的處理規則**：報價單中若一個單價包含多個子項目（例如「冰水管安裝及配管 1式 175,100」下含10項細目），**不得跳過或留空**，應：
...
> 此清單僅列常見案件，非完整選項；實際選擇仍以資料庫現有 select 選項為準（見 STEP 3）。
