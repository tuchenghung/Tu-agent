...
#### 2.5 確認案件財務管理頁面成本已同步（不建、不寫任何獨立子表格）

> ⚠️ **2026-07-14 二次修正**：先前版本（2026-07-08）曾指示「若找到欄位為工種/品項、廠商、合約金額、已付金額、狀態、備註的獨立子資料庫，就寫入一筆」——這個判斷本身沒錯，但**處置方式錯了**。西湖辦公室排水修改工程案就是一個依此規則產生的獨立「本案發包明細」子資料庫，資料與發包管理主庫重複但完全脫鉤，2026-07-14 造成使用者混淆才發現。**正確做法：只要發包管理記錄的 `專案` relation 填對，案件財務管理頁面的「發包項目」relation 與「成本」rollup 就會自動反映，不需要、也不應該再寫入任何獨立子表格。**

**執行方式：**

1. 完成 STEP 8-2 建立發包管理記錄（含正確的 `專案` relation）即結束，直接跳到 STEP 8-3 回報，不用額外確認或寫入案件財務管理頁面的任何子表格
2. 若使用者質疑「表格裡怎麼沒看到」，說明：案件財務管理頁面若沒有獨立子資料庫，「發包項目」relation 本身就是即時清單，用 `API-retrieve-a-page` 讀該頁面即可看到最新的 `發包項目` 與 `成本` 欄位
3. 若用 `API-get-block-children` 意外發現頁面內已存在一個 `type: "child_database"` 且欄位為「工種/品項、廠商、合約金額、已付金額、狀態、備註」的表格 → **這是舊規則遺留的重複表格，不要往裡面寫新資料**。告知使用者這是技術債（與發包管理主庫脫鉤），建議刪除該子表格（刪除前需使用者確認），不要繼續維護兩套資料
