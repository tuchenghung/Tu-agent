...
2. **資產配置：主動 vs 被動小表格**（2026-08-19起每次必做，見下方「資產配置：主動 vs 被動」規則）
3. **本週最大拖累**（跌幅較大或轉虧個股，附原因）
4. **加碼回顧**（本次有加碼時，逐筆評斷 ✅/⚠️/❌）
5. **個股操作建議**
   - 停損評估（虧損較深或停滯個股）
   - 續抱理由（核心獲利部位）
6. **一句話結論**（callout 藍色）
...
同台股 STEP 7 邏輯（含「資產配置：主動 vs 被動」小表格）。美股額外注意：
- 損益換算需同時呈現 USD 與 TWD
- 加碼回顧含匯率影響分析
- 「資產配置：主動 vs 被動」小表格市值欄用 USD（不需 TWD 換算）
...
## 資產配置目標比例：100-年齡法則（2026-08-19新增，僅台美保單合計頁適用）

使用者採用「100-年齡法則」設定主動/被動目標配置：**目標主動% = 100-年齡，目標被動% = 年齡**（使用者目前約42歲 → 目標主動58%／被動42%，逐年需重新核算）。**此目標比例只適用於台美保單合計總資產，不套用在台股/美股單一市場頁。**

**STEP 8（台美合計統計）「資產配置：主動 vs 被動」小表格，需擴充為5欄**（僅合計頁，台股/美股個別頁維持3欄簡版）：
類型 / 市值NT$ / 實際比例 / 目標比例（100-年齡法則）/ 差距

並在表格後加一個紫色🎯 callout，說明目標法則公式、現況超配/低配百分點與需轉移的金額（現況比例−目標比例）×總資產。

---

## 資產配置比例：現金／股票／債券／公司股權投資（2026-08-20新增，僅台美保單合計頁適用）

**與「主動 vs 被動」是兩套獨立分類系統，互不相關，兩個表格都要各自維護、不合併。**

**分類規則**：
| 類別 | 涵蓋範圍 |
|---|---|
| 現金 | 使用者手上活存/定存，**目前無自動化來源追蹤**，每次需向使用者詢問最新現金餘額 |
| 股票 | 台股全部個股 ＋ 美股個股（非ETF）＋ 美股股票型ETF（VTI/VGK/VPL/VWO）＋ 保單股票性質基金 |
| 債券 | 美股債券型ETF（VGIT/BWX）＋ 保單債券性質基金 |
| 公司股權投資 | 未上市公司股權投資，**目前無自動化來源追蹤**，每次需向使用者詢問最新持股/認購金額，2026-08-20起已知一筆：宏洋 NT$200,000 |

**保單4檔基金依屬性拆分規則**（2026-08-20建立，未來新增/調整基金時比照）：
- A24-柏瑞印度股票基金、F15-富達東協基金 → 股票（股票型基金）
- AC3-聯博-美國收益基金A2（高風險非投資等級債券）→ 債券（名稱已明確標註債券屬性）
- ML5-貝萊德環球資產配置基金A2 → **股債混合多資產基金，無法查到精確配置比例，概估整筆歸入股票**，每次沿用此概估法，除非使用者提供該基金月報實際股債配置比例可重新拆算

**執行方式**（STEP 8 台美合計統計，僅合計頁）：
1. 用 `mcp__notion__API-patch-block-children` 在頁面新增 `heading_3（📊 資產配置比例（現金／股票／債券／公司股權投資）) + table（4欄：類別/金額NT$/佔比/組成）`
2. 5列：現金／股票／債券／公司股權投資／合計（合計列金額=100%，缺一不可，2026-08-20曾被使用者提醒補上）
3. 表格後加一個黃色⚠️ callout，說明保單拆分規則（尤其ML5概估歸股票這點必須每次註明，避免被誤認為精確數字）
4. 因現金與公司股權投資皆無自動化來源，**每次執行本段落前必須先問使用者這兩項的最新金額**，不可沿用上次數字

---

## 美股帳戶股債比目標：80/20（2026-08-20新增，僅美股頁面適用）

使用者設定美股帳戶內部股債比目標為 **80股/20債**，與「主動vs被動」「現金/股票/債券/公司股權投資」是第三套獨立分類系統，只適用於美股頁面。

**分類規則**：VTI/VGK/VPL/VWO（股票型ETF）與非ETF個股 → 股票；VGIT/BWX（債券型ETF）→ 債券

**執行方式**（STEP 7美股評估報告，僅美股頁面）：
1. 用 `mcp__notion__API-patch-block-children` 新增 `heading_3（股債比：現況 vs 目標（80/20）) + table（4欄：類別/市值USD/實際比例/目標比例）`
2. 2列：股票（股票型ETF＋個股）／債券（債券型ETF）
3. 表格後加紫色🎯 callout，說明分類規則、現況與目標差距、需轉移金額（USD及約略TWD換算）
4. **中文內容寫入後務必逐字讀回核對**——2026-08-20實測連續3次咬字：「個股」→「个股」、「其餘」→「其余」，皆為簡體/日文變體字，反覆咬同一詞就換完全不同措辭重寫（例如把「其餘單一股票」改成「非ETF股票」），不要只是重試同一句話

---

## 資產配置：主動 vs 被動（2026-08-19新增，每次快照評估報告必做）

**背景**：2026-08-19使用者要求在美股投資評估報告新增「主動股票 vs 被動指數基金」分配比例，並指示台股比照辦理，未來每次快照都要呈現。因 Notion table_width 建立後不可修改，若直接在既有21/28列持倉明細表加欄位需整表刪除重建、風險高，改採方案：在「整體指標」表格區塊旁新增一個獨立小表格，不動明細表結構。

**判斷規則**：
- **被動（指數ETF）**：追蹤大盤/區域/債券指數的被動型ETF，目前美股持倉中為 **VTI、VGK、VPL、VWO、VGIT、BWX**（Vanguard系列，STEP 6已列為ETF清單）
- **主動（個股）**：以上6檔以外的所有個股（含AI/半導體、太空國防、核能儲能、AI機器人自動化等主題個股）
- **台股ETF/指數基金持倉判斷（2026-09-12修正）**：元大台灣50（0050）等追蹤大盤指數的ETF＝被動；其餘台股個股全數歸類主動。台股自2026-08-23起已持有元大台灣50，先前「台股全數主動」規則僅適用於此之前的快照，之後每次快照需重新檢查台股持倉是否含指數型ETF再分類，不可預設全部主動。

**執行方式**：
1. 用 `mcp__notion__API-patch-block-children` 在該次投資週報頁面「整體指標」表格之後，插入 `heading_3（資產配置：主動 vs 被動）+ table（3欄）`
2. 表格欄位：類型 / 市值USD（美股）或市值TWD（台股）/ 比例
3. 兩列：被動指數ETF（或主動個股，台股列在前）
4. 數值依當次持倉快照的市值加總計算，比例取小數點後1位
