...
TEMPLATES = {
    ...
    '中型_羅東聖母史蹟館版': (r'D:\Dropbox\Tu-agent\000_Agent\templates\報價單範本_中型_羅東聖母史蹟館版.xlsx', '靈醫會史蹟館'),
...
**規模判斷門檻（2026-08-30使用者明確定義）：大型＝合約總額1000萬以上；未達1000萬（含約669萬等級）算中型，不是大型。** 之前曾誤把一個約669萬的案子當大型範本存放，已修正移除，判斷規模時務必先看合計金額再決定歸類，不要用「品項多寡/類別數量」猜測規模。

> ⚠️ **大型範本尚未建立**：告知使用者需先手動儲存一份千萬元以上等級的乾淨版/實際案例 Excel 作為範本至 `templates\報價單範本_大型.xlsx`

**中型級別現有兩個範本選項（2026-08-30起，使用者明確要求兩個並存，不要互相取代）：**
1. `報價單範本_中型.xlsx`（原有，耳鼻喉科V3格式，乾淨空白版，支援 STEP 4 `{{xxx}}` 自動替換）
2. `報價單範本_中型_羅東聖母史蹟館版.xlsx`（新增，來源：羅東聖母醫院靈醫會史蹟館案，合約約669萬，甲.壹~甲.柒完整7大類，含消防/空調廠商報價實際填入成本區的完整範例）。**這份刻意保留完整實際數字**（業主名稱、案件金額、廠商品項/單價全部是真實資料，不是清空的空白範本），使用者明確指示要留著當「已完成的參考範例」。
   - **不能直接套用 STEP 4 的 `{{xxx}}` 自動替換流程**（win32com 找不到 placeholder 標記會失敗）——複製這份後要當作格式/公式寫法的參考範例，手動改寫業主/工程名稱/各項品名數量單價，或改用 `廠商報價填入預算表` skill 的方法重新填入新案件的實際廠商報價
   - 工作表名稱固定是「靈醫會史蹟館」，複製後記得依新案件改工作表名稱

新建報價單時若規模判斷為中型，用 AskUserQuestion 讓使用者選要用哪一個中型範本（空白版 vs 實際範例版），不要自行預設。
