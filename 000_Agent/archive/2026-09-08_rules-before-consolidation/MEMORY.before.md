<!-- AI 分身起始助手紀錄:START -->
<!-- AI 分身起始助手 by 雷小蒙 v1.2 · 2026-05-06 · by 雷蒙（Raymond Hou）· https://github.com/Raymondhou0917/claude-code-resources · CC BY-NC-SA 4.0 -->

# 裝修工程接案主管的 AI 分身記憶

> 這裡存我跟 AI 之間跨 session 的偏好、經驗、踩坑紀錄。
> AI 每次 session 開始會自動讀這個檔案。

---

## 用戶偏好

- **共用資料根目錄（2026-09-07）**：使用者指定 `D:\Dropbox\Tu-agent` 為 AI Agent 資料存放資料夾，Markdown 資料及未來新增的工作資料皆存放於此，依既有資料層路由分類。Claude Code 也使用同一份資料；修改共用檔案前須重新讀取，避免覆蓋其他工具的新內容，並遵守 Windows / macOS 設定隔離原則。

---

## Feedback（AI 學到的原則）

- **Agent 分工澄清（2026-09-07）**：過度以兩個 Agent 同時修改同一任務為前提提醒 → 使用者會讓 Codex 與 Claude Code 處理不同任務，按此分工正常共用資料 → 不必反覆提醒同任務衝突；共用記憶與規則檔仍須在修改前讀取最新內容。
- **雙系統隔離原則**：使用者同時有 Windows 11（PC）與 macOS（MacBook Pro M3 Pro），兩邊透過 Dropbox 同步專案資料夾。在 Windows 操作時不得修改 Mac 專屬設定（如 `~/.nvm/`、Mac 路徑的 MCP 設定）；在 Mac 操作時同理不動 Windows 設定（如 `C:\Users\deco01\nodejs\`、`-win` 結尾的 MCP 設定）。設定檔若有平台差異，以當前執行平台為準，不要跨平台套用。
- [建立 Notion 資源前先查重](feedback_check_before_create.md) — 建 database/page 前用 `API-get-block-children` 確認頁面上無同名資源，避免重複建立。

---

## 工作流 SOP

### /project-import — 工程專案資料夾匯入
- **觸發**：使用者提供 Dropbox 專案資料夾路徑
- **功能**：自動掃描 → 依 A-I 分類移檔 → 建 Notion 專案頁面 → 建折疊標題
- **Skill 檔**：`.claude/commands/project-import.md`
- **關鍵**：heading_2 toggleable 需用 Node.js 直接呼叫 Notion API（MCP 工具不支援）
- **Notion Projects DB**：`3355cac1-0351-4ef4-8eb1-8b8f0bb619c3`
- **實際驗證案例**：羅東聖母醫院S棟5樓耳鼻喉科（2026-05-15）、南投基督教醫院（2026-05-15）

---

## Dropbox API

| 項目 | 值 |
|---|---|
| 申請頁面 | https://www.dropbox.com/developers/apps |
| Token 有效期 | 約 4 小時，需手動重新產生 |
| Token 位置 | Settings → OAuth 2 → Generate access token |
| 必要 Permission | `sharing.write`（在 Permissions 頁籤勾選）|
| 建立/取得分享連結腳本 | `D:\Dropbox\Tu-agent\000_Agent\scripts\dropbox_notion_links.mjs` |

---

## 無塵室空調專案知識庫（2026-06-03 建立）

參考資料路徑：`400_Knowledge/工程/建材規格/`

| 知識文件 | 重點內容 |
|---------|---------|
| `2026-06-03_MAU外氣處理機規格與報價參考.md` | 弘振/勝新 MAU 報價（2010，矽品）；元/CMM 分析；組裝費 |
| `2026-06-03_AHU空調箱規格與報價參考.md` | 弘振 AHU 全機型（1400~80CMM），元/CMM 分析，2011年未稅 |
| `2026-06-03_水冷式冰水主機規格與報價參考.md` | 日立 RCU-409WS，28RT/30kW，467,800/台含稅（2011） |
| `2026-06-03_氣冷分離式冷氣機規格與報價參考.md` | 大同 TFP 系列完整規格，8RT=90,000/組含稅（2011） |
| `2026-06-03_空調箱濾網規格與報價參考.md` | 晨鼎/AIRREX；初效 75/只，袋型 770/只（2011） |
| `2026-06-03_起重設備安裝定位費用參考.md` | AHU 16,000/台，全廠一批 19~22萬含稅（2011） |
| `2026-06-03_冷氣單位換算表.md` | 1RT=2,500kcal=10,000BTU≈2.9kW，台灣慣用值 |
| `2026-06-03_三相380V馬達電力配線規格表.md` | CNS-2934 vs Appendix D FLA 對比；EMT 選型；法規214/220條 |
| `2026-06-03_無塵室空調監控系統IO點數表.md` | SP-IC-FO01 DXF 解析；118點；每台MAU=22點；DDC架構 |
| `2026-06-03_無塵室空調控制架構圖說參考.md` | STAND-ALONE DDC；MAU/乾盤/FFU/排氣控制點位圖 |
| `2026-06-03_空調配管施工規範重點.md` | 管材/保溫厚度/水壓試驗/吊架間距/冰水主機R-134a規格 |
| `2026-06-03_無塵室空調工程報價計算方式_博訊龍潭廠.md` | **2025年最新報價**；佑祥老余；MAU 275,000/台；冰機 220,000台；FFU 13,000/台；配管/風管工資利潤率45~65%；螺旋風管成本表；鐵皮張數計算公式 |
| `2026-06-03_空調工程技師事務所報價格式_國軍台中總醫院.md` | 技師事務所預算書格式；7大工種含TAB(2.5%)；材料/工資分開計；電動三通閥1,100/組；控制工程費205,100；水冷冰機13,333/RT（2005-2010年估）|
| `2026-06-03_空調規劃基礎設計參數與造價基準.md` | **規劃基礎**；RT計算公式；鐵皮換算(1張=21才)；換氣次數(C/R-10K=30~35次)；造價基準(MEP=30,000/RT成本；一般空調=2,000/M²成本)；2025修正×2.0~2.5 |
| `2026-06-03_FFU風車濾網箱規格與報價參考.md` | Air Tech FFU-T15M；1200×600mm T-Bar；15CMM/台；ULPA 0.3μm 99.97%；200VA；2011年6,300/台(不含濾網)；2025估13,000~18,000/台(含ULPA)；Air Shower 2011年125,000/台 |
| `2026-06-03_全套空調規劃圖分析_803圖_108年版.md` | **完整設計圖（含SLD+大樣圖）**；AH+FCU系統；水冷2×30RT=60RT；冰水7/12°C；風管45→35→25→20cm縮管；水管保溫厚度表A；帆布接頭尺寸；DDC比例閥控制說明；全案負載70KW |
| `2026-06-03_AHU_MAU選用設計方法論.md` | **設計方法核心**；PAH/AHU/RCU/MAU定義；AHU vs MAU設計差異；SA/OA/RA計算；1RT=14.17CMM；混風公式；熱水盤管=CMM×17.4×ΔT÷3024；加濕量計算 |
| `2026-06-03_機電圖檔系統配置分析.md` | 完整機電圖24頁；11.4kV高壓→TR→ACMP/ML1/MLJ配電；ML1總容量415,388VA/631A；排水管4"主幹/2"分支/S=1:50/45°彎；給水PVC規格 |
| `2026-06-03_空調水管尺寸選用表_GIP.md` | **GIP管選管查表**；冰水2.4GPM/RT，冷卻水3GPM/RT；3/4"~14"完整RT範圍；含Python選管程式碼 |
| `2026-06-03_EMT導線管規格表_CNS2606.md` | **CNS 2606 C4060實際內徑**；E-19(16.7)~E-75(72.6mm)；40%填充面積計算；Python選管程式碼 |
| `2026-06-03_空調標單圖面範例_矽品CHC案分析.md` | **完整30頁無塵室施工圖範本**；矽品G-PROJECT；MAU(800CMM/303RT/11500×4760mm)+RCU×3(1290CMM/280kW/台)+乾盤×11(220CMM/26.3kW)+FFU×160(15CMM/185W/1170×570mm)；總冷卻~624RT |

**通膨修正係數（2026-06-03 更新）**：
- 2005~2010年報價 → 2025年：管材×2.2~2.5、工資×2.2~2.8、設備×1.8~2.2、控制元件×1.5~1.8
- 2011年報價 → 2025年：×1.8~2.2（×1.5已嚴重低估，不得使用）
- 校準基準：老余2025年2月博訊龍潭廠實際報價

**程式待更新（等「更新」指令）**：
- FLA 表：從 Appendix D 改為 CNS-2934（法規依據）
- 導線截面積：從 FLA 直接查表改為 FLA×1.25（法規第214條）
- EMT 選管：改為填充率計算（40%填充，實際管內徑）
- MAU 單價：275,000/台 → 依 CMM 分級計算
- 乾盤單價：45,000/台 → 35,000/台（2025年估）

---

## 踩坑筆記

- `mcp__notion-win__API-patch-block-children` 只支援 `paragraph` / `bulleted_list_item`，無法建立 `heading_2 toggleable`。需用 Node.js + Notion REST API 直接建立。

---

## 環境速查表

| 項目             | 值                                             |
| :--------------- | :--------------------------------------------- |
| AI 分身母資料夾  | `/Users/tuzhenghong/Tu-agent`（symlink → `~/Dropbox/Tu-agent`） |
| 建立日期         | `2026-04-30`                                   |
| Skills symlink   | ✅ `~/.claude/skills` → `000_Agent/skills/`    |
| 記憶系統啟用     | ✅                                             |
| 日記功能         | ✅ `300_Journal/` 已建立                       |

<!-- AI 分身起始助手紀錄:END -->

## 郵政大樓 3D 專案（2026-09-08）
- 使用者確認 20260618一層隔間配置圖.dxf 為郵政大樓一樓平面圖，未來會持續製作郵政大樓 3D 專案。目前先保存，等待後續指示。
- 專案入口：100_Todo/projects/postal-3d/專案紀錄.md；內含來源 DXF 備份、5m 牆柱 DXF/OBJ、預覽及完工配色示意。米白牆／灰藍柱／淺灰地坪為提案配色，非正式確認選色。後續以來源 DXF 尺寸為準，勿依 AI 圖反推尺寸。


- **正式存放位置（使用者最新指定，2026-09-08）**：D:\Dropbox\宏祐\施工中案件\20260618-中華郵政臺北郵件處理中心整修統包工程\K施工圖面\3D專案。往後郵政大樓 3D 成果存此資料夾，入口為其中的 專案紀錄.md；優先於先前 Tu-agent 工作位置。已複製 17 個工作檔與 source/ 原始 DXF；Tu-agent 保留備份。


## 子代理模型調度授權（2026-09-08）
- 使用者明確授權：「允許你依任務難易度，自行選擇不同模型的子代理；簡單任務優先省用量，工程精度優先品質。」
- 在當次環境允許且模型可用的範圍內，自行選擇子代理模型與推理程度，不必為每次一般分工重複詢問。
- 簡單工作可由主代理直接完成，避免不必要的分派開銷；有獨立且適合分工的子任務才啟用子代理。
- 工程尺寸、規格、數量與建模精度優先品質，由主代理整合並驗證結果。
- 此授權不代表可自行切換正在執行的主對話模型，也不包含購買額度、付費訂閱或對外傳送訊息。

## AI 協作角色偏好（2026-09-08）
- 使用者指定 AI PM、AI Tech Lead、Workflow Architect 角色；完整規則已整合於根目錄 AGENTS.md 的「AI 協作角色與工作流程」。複雜工作採模組化流程與適當子代理分工，簡單工作直接處理；按需載入脈絡，避免重複常駐專案細節。繁體中文採台灣常用詞彙。

